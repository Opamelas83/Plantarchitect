...
MyArchidata %>% filter(!studyDesign %in% c("Lattice", "splitplot")) %>% group_by(studyDesign) %>% 
     summarize(N_trials=length(unique(studyName)))# remove TARI and trial design = Lattice and splitplot represented by 1 and 2 trials
#"BranchlevelNum", "FirstBranchHeight", "PlantHeight", "PlantArchitecture # unique(MyArchidata$studyDesign)
...
MyArchidata %<>% 
  filter(!studyDesign %in% c("Lattice", "splitplot")) %>%
  filter(!is.na(replicate)) %>%
  filter(!is.na(germplasmName)) %>%
  filter(observationLevel == "plot") %>%
     group_by(studyName) %>% 
